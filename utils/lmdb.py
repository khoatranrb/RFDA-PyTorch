...
def make_lmdb_from_imgs_v2(img_dir,
                        lmdb_path,
                        img_path_list,
                        keys,
                        batch=5000,
    ...
    if multiprocessing_read:
        def _callback(arg):
            ...
            # key, img_byte, img_shape = arg
            for ar in arg:
                path,key, img_byte, img_shape = ar
                new_path_keys.append((path,key))
                dataset[key], shapes[key] = img_byte, img_shape
                pbar.set_description(f'Read {key}')
                pbar.update(1)
        ...
        for path, key in zip(img_path_list, keys):
            pool.apply_async(
                _read_img_worker_v2,
                args=(op.join(img_dir, path),path, key, compress_level),
                callback=_callback
                )
        pool.close()
        pool.join()
    ...
    if map_size is None:
        ...
        data_size_per_img = img_byte.nbytes
        data_size = data_size_per_img * len(img_path_list)
        map_size = data_size * 5  # enlarge the estimation
    ...
    # for idx, (path, key) in enumerate(zip(new_img_path_list, new_keys)):
    for idx, (path, key) in enumerate(new_path_keys):
        pbar.set_description(f'Write {key}')
        pbar.update(1)
        ...
        if multiprocessing_read:
            ...
            _, img_byte, img_shape = _read_img_worker_v2(
                op.join(img_dir, path), key, compress_level
                )  # use _read function
            h, w, c = img_shape
...
def _read_img_worker_v2(path1,path, key, compress_level):
    """Read image worker.
    ...
    if img.ndim == 2:
        ...
        return (path,key, img_byte, (h, w, c))
    else:
        h, w, c = img.shape
        base=w//4#960//4=240
        target_w=base*2#480
        to_return=[]
        for i in range(0,target_w+1,base):#0,240,480+480
            tmp_img=img[:,i:i+target_w]
            _, img_byte = cv2.imencode(
                '.png',tmp_img , [cv2.IMWRITE_PNG_COMPRESSION, compress_level]
            )
            to_return.append((path,key+"%s"%i, img_byte, (h, tmp_img.shape[1], c)))
        return to_return
